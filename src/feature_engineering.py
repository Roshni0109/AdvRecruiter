...
def compute_final_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Why? An additive weight of 0.20 is too weak — a Civil Engineer with
    # lots of AI keywords can still score 0.48 even with title_relevance=0.
    #
    # This multiplier means:
    #   title_relevance = 0.0  → final = base * 0.35  (65% cut — strong penalty)
    #   title_relevance = 0.25 → final = base * 0.57  (43% cut — moderate penalty)
    #   title_relevance = 0.80 → final = base * 0.87  (small penalty)
    #   title_relevance = 1.0  → final = base * 1.00  (no penalty)
    #
    # This ensures HR Managers and Civil Engineers can NEVER rank above genuine
    # AI/ML engineers, no matter how many AI keywords they've stuffed in.
    ...
    # Use iloc[-1] (last row) instead of iloc[99] so this works
    # whether we have 50 candidates (testing) or 100,000 (full run)
